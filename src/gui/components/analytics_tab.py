...
class AnalyticsTab(tk.Frame):
    ...
    def _build_ui(self):
        ...
        self.lbl_total_tenders = make_card(cards_fr, 0, "Total Tenders", TEXT, self.show_all_tenders)
        self.lbl_matching_wants = make_card(cards_fr, 1, "Matching Wants", SUCCESS, self.show_matching_wants)
        self.lbl_filtered_dont_wants = make_card(cards_fr, 2, "Filtered Don't Wants", ERR, self.show_filtered_dont_wants)
        self.lbl_not_filed = make_card(cards_fr, 3, "Not Filed Wants", WARN, self.show_not_filed_wants)
        self.lbl_firm_matched = make_card(cards_fr, 4, "Firm Matched Tenders", "#84D2FF", self.show_firm_matched)
        
        # Middle row for charts
        ...
        self.firm_chart_canvas.bind("<Configure>", lambda e: self.redraw_firm_chart())
    ...
    def update_analytics(self):
        ...
        for r in self.app._records:
            ...
            if is_want:
                ...
                if end_str:
                    dt = CalendarTab._parse_date_str(end_str)
        ...
        if not display_deadlines:
            ...
            for dt, end_str, r in display_deadlines:
                ...
                def make_click(b_no=bid_no):
                    return lambda e: self.app.calendar_tab.locate_in_table(b_no)
    ...
    def redraw_chart(self):
        ...
        for idx, (min_name, count) in enumerate(self._last_ministry_data):
            ...
            def make_enter_handler(bt=bar_tag):
                return lambda e: [
                    self.chart_canvas.itemconfig(bt, fill="#58A6FF"),
    ...
    def _filter_by_ministry(self, name):
        self.app.calendar_tab.locate_in_table(name) # Switches tab, resets filters and searches name

    def show_all_tenders(self):
        self.app.notebook.select(self.app.tab_table)
        if hasattr(self.app, "view_var"):
            self.app.view_var.set("All Tenders")
        if hasattr(self.app, "status_view_var"):
            self.app.status_view_var.set("All")
        if hasattr(self.app, "search_var"):
            self.app.search_var.set("")
        if hasattr(self.app, "date_filter_preset_var"):
        ...
        self.app._log("info", "Filtered Table View to all tenders.")

    def show_matching_wants(self):
        self.app.notebook.select(self.app.tab_table)
        if hasattr(self.app, "view_var"):
            self.app.view_var.set("Wants (Matches)")
        if hasattr(self.app, "status_view_var"):
            self.app.status_view_var.set("All")
        if hasattr(self.app, "search_var"):
            self.app.search_var.set("")
        self.app._refresh_table_view()
        self.app._log("info", "Filtered Table View to matching wants.")

    def show_filtered_dont_wants(self):
        self.app.notebook.select(self.app.tab_table)
        if hasattr(self.app, "view_var"):
            self.app.view_var.set("Don't Wants (Filtered)")
        if hasattr(self.app, "status_view_var"):
            self.app.status_view_var.set("All")
        if hasattr(self.app, "search_var"):
            self.app.search_var.set("")
        self.app._refresh_table_view()
        self.app._log("info", "Filtered Table View to filtered don't wants.")

    def show_not_filed_wants(self):
        self.app.notebook.select(self.app.tab_table)
        if hasattr(self.app, "view_var"):
            self.app.view_var.set("Wants (Matches)")
        if hasattr(self.app, "status_view_var"):
            self.app.status_view_var.set("To Be Filed")
        if hasattr(self.app, "search_var"):
            self.app.search_var.set("")
        self.app._refresh_table_view()
        self.app._log("info", "Filtered Table View to wants not yet filed.")

    def show_firm_matched(self):
        self.app.notebook.select(self.app.tab_table)
        if hasattr(self.app, "view_var"):
            self.app.view_var.set("All Tenders")
        if hasattr(self.app, "status_view_var"):
        ...
        for iid in self.app.tv.get_children():
            if self.app.tv.set(iid, "matched_firm").strip():
                matched_rows.append(iid)
        if matched_rows:
            self.app.tv.selection_set(matched_rows)
            self.app.tv.see(matched_rows[0])
        self.app._log("info", "Highlighted firm-matched tenders in Table View.")
    ...
    def _filter_by_firm(self, name):
        ...
        if hasattr(self.app, "search_var"):
            self.app.search_var.set(name)
            
        self.app._refresh_table_view()
        self.app._log("info", f"Filtered Table View to Firm: '{name}'")
